...
def take_selfie():
    """API endpoint to take a selfie using DroidCam"""
    try:
        preview_url = "http://172.20.24.0:4747/video"
        snapshot_url = "http://172.20.24.0:4747/snapshot"

        # Activate preview by opening a GET request (stream)
        try:
            preview_stream = requests.get(preview_url, stream=True, timeout=2)
        except Exception as e:
            return jsonify({'error': f'Failed to open preview stream: {e}'}), 500

        # Wait for 5 seconds while keeping preview alive
        start_time = time.time()
        try:
            for _ in preview_stream.iter_content(chunk_size=1024):
                if time.time() - start_time >= 5:
                    break
        except Exception:
            pass  # Timeout or disconnect is okay

        # Capture actual selfie image
        try:
            snapshot_response = requests.get(snapshot_url, timeout=3)
            ...
            return jsonify({'error': f'Error capturing snapshot: {e}'}), 500
